...
import loggers
from sql.mapping import TableObject
from .. import compat
from ..errors import ModelError, InternalError, ArgumentError, HierarchyError
from ..model.utils import object_dict
...
class StarSchema(object):
    ...
    * `model` is a SQLAlchemy model object where the snowflake tables
      are described.
    ...
    * `tables` are SQL Alchemy selectables (tables or statements) that are
      ...
      the actual model. Only table name has to be specified and database
    ...
    * a dictionary with keys or object with attributes: `master`, `detail`,
    ...
    def __init__(self, label, metadata, mappings, fact, fact_key='id',
        ...
        self.logger = loggers.getLogger("cubes_lite.starschema")
    ...
    def _collect_tables(self):
        ...
        * `join` – join object that joins the table as a detail to the star
        ...
        #
        fact_table = TableObject(schema=self.schema,
                                 name=self.fact_name,
