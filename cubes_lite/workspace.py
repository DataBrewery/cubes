...
from .model import read_model_metadata, find_dimension
...
class Workspace(object):
    ...
    def _store_for_model(self, metadata):
        """Returns a store for model specified in `model`. """
    ...
    def import_model(self, model=None, provider=None, store=None,
        ...
        model dictionary, filename, path to a model bundle directory or a
        URL.
        ...
        If not specified, then the one from the model dictionary will be
        used.

        Model's provider is registered together with loaded model. By
        default the objects are registered in default global namespace.

        Note: No actual cubes_lite or dimensions are created at the time of calling
        this method. The creation is deferred until
        :meth:`cubes_lite.Workspace.cube` or :meth:`cubes_lite.Workspace.dimension` is
        called.
        """
        # 1. Metadata
        # -----------
        # Make sure that the model is a dictionary
        # 
    ...
    def cube_names(self, identity=None):
        """Return names all available cubes_lite."""
        return [cube["name"] for cube in self.list_cubes()]

    # TODO: this is not loclized!!!
    def list_cubes(self, identity=None):
        """Get a list of model for cubes_lite in the workspace. Result is a list
        ...
        cubes_lite for `identity` and only authorized list of cubes_lite is returned.
        """
    ...
    def cube(self, ref, identity=None, locale=None):
        ...
        if lookup:
            ...
            trans = context.object_localization("cubes_lite", cube.name)
