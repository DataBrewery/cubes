...
class SQLExpressionTestCase(SQLTestCase):
    ...
    def setUpClass(self):
        ...
        data = [[1, 10, 1], [2, 20, 1], [3, 40, 2], [4, 80, 3]]

        for row in data:
            self.engine.execute(insert.values(row))

        self.bases = ["id", "price", "quantity"]
        self.columns = {attr: self.table.columns[attr] for attr in self.bases}
    ...
    def test_instance(self):
        self.assertIsInstance(self.compiler.compile("id", self.context), ColumnElement)

        self.assertIsInstance(self.compiler.compile("1", self.context), ColumnElement)

        self.assertIsInstance(
            self.compiler.compile("'text'", self.context), ColumnElement
        )

        self.assertIsInstance(
            self.compiler.compile("1 + 1", self.context), ColumnElement
        )

        self.assertIsInstance(
            self.compiler.compile("'text' + 1", self.context), ColumnElement
        )
    ...
    def test_with_constant(self):
        ...
        self.assertExpressionEqual(self.table.columns["price"] + 1, column)

        column = self.compiler.compile("price * 10 + 1", self.context)
        self.assertExpressionEqual((self.table.columns["price"] * 10) + 1, column)

    def test_multiple_columns(self):
        column = self.compiler.compile("price * quantity", self.context)
        self.assertExpressionEqual(
            self.table.columns["price"] * self.table.columns["quantity"], column
        )
    ...
    def test_function(self):
        ...
        self.assertExpressionEqual(sa.func.min(self.table.columns["price"], 0), column)
