...
class CombinationsTestCase(unittest.TestCase):
    ...
    def test_levels(self):
        combos = cubes.utils.combine_nodes([self.nodea])
        self.assertEqual(len(combos), 3)

        combos = cubes.utils.combine_nodes([self.nodeb])
        self.assertEqual(len(combos), 2)

        combos = cubes.utils.combine_nodes([self.noded])
        self.assertEqual(len(combos), 1)

    def test_combos(self):
        combos = cubes.utils.combine_nodes([self.nodea, self.nodeb])
        self.assertEqual(len(combos), 11)

        combos = cubes.utils.combine_nodes([self.nodea, self.nodeb, self.nodec])
        self.assertEqual(len(combos), 35)
	
    def test_required_one(self):
        nodes = [self.nodea, self.nodeb, self.nodec]
        required = [self.nodea]
        combos = cubes.utils.combine_nodes(nodes, required)
